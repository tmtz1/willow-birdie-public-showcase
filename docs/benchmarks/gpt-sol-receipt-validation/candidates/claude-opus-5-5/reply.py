...
def _reject_duplicate_keys(pairs: list[tuple[str, object]]) -> dict[str, object]:
    """Refuse ambiguous receipts such as {\"accepted\": false, \"accepted\": true}."""
    obj: dict[str, object] = {}
...
def _is_confirmed_receipt(stdout: object) -> bool:
    """Confirm only one JSON object with accepted=true and a 64-char lowercase hex event_id."""
    if not isinstance(stdout, str):
        return False
    try:
        receipt = json.loads(stdout, object_pairs_hook=_reject_duplicate_keys)
    ...
    return (
        ...
        and re.fullmatch(r"[0-9a-f]{64}", event_id) is not None
    )
...
def send_reply(
    ...
    *,
    ...
    # Exit status 0 alone is not confirmation; require a well-formed receipt on stdout.
    if not _is_confirmed_receipt(getattr(result, "stdout", None)):
        raise RuntimeError("Buzz did not confirm the reply; do not retry this event blindly.")
