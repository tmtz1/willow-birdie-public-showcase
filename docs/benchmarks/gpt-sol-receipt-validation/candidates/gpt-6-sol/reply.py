#!/usr/bin/env python3
"""Single-purpose, send-only Buzz reply tool for the Genie ACP profile."""
    ...
    *,
    ...
    with _attempt_lock:
        ...
        try:
            ...
            raise RuntimeError("Buzz send timed out; check the channel before attempting another reply.") from None
        except OSError:
            raise RuntimeError("Buzz sender could not start; reply was not confirmed.") from None
    unconfirmed = "Buzz send was not confirmed; do not retry this event blindly."
    if result.returncode != 0 or not isinstance(result.stdout, str):
        raise RuntimeError(unconfirmed)
    try:
        receipt = json.loads(result.stdout, parse_constant=_reject_json_constant)
    except ValueError:
        raise RuntimeError(unconfirmed) from None
    if (
        not isinstance(receipt, dict)
        or receipt.get("accepted") is not True
        or not isinstance(receipt.get("event_id"), str)
        or re.fullmatch(r"[0-9a-f]{64}", receipt["event_id"]) is None
    ):
        raise RuntimeError(unconfirmed)
